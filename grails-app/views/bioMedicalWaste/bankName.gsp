    ...
    <%
				...
				String date1 = new SimpleDateFormat("dd-MM-yyyy hh:mm").format(new Date());
				 ...
				 def feesOnline = FeeBankOtherDetailsBMW.findAll("from FeeBankOtherDetailsBMW fee where fee.application=?",[indApplicationDetailsInstance])
				  ...
				  def misc1 = "0"
				  def interest = "0"
				  if(feesOnline.feeFor.contains('BMW'))
				{
				
					def feesOnline1 = FeeBankOtherDetailsBMW.find("from FeeBankOtherDetailsBMW fee where fee.feeFor='BMW' and fee.application=?",[indApplicationDetailsInstance])
    	  		...
    	  		if(feesOnline.feeFor.contains('Application Form'))
    	  		{
    	  			...
    	  			def feesOnline2 = FeeBankOtherDetailsBMW.find("from FeeBankOtherDetailsBMW fee where fee.feeFor='Application Form' and fee.application=?",[indApplicationDetailsInstance])
    	  		...
    	  		if(feesOnline.feeFor.contains('Balance Fee'))
    	  		{
    	  			...
    	  			def feesOnline3 = FeeBankOtherDetailsBMW.find("from FeeBankOtherDetailsBMW fee where fee.feeFor='Balance Fee' and fee.application=?",[indApplicationDetailsInstance])
    	  			
    	  			misc = feesOnline3.amount
    	  		}
    	  		
    	  		
    	  			def feesOnline4 = FeeBankOtherDetailsBMW.find("from FeeBankOtherDetailsBMW fee where fee.feeFor!='Application Form' and fee.feeFor!='BMW' and fee.feeFor!='Balance Fee' and fee.application=?",[indApplicationDetailsInstance])
		...
		System.out.println("e-----"+encdata1);
		System.out.println("e-----"+encdata2);
...
System.out.println("String Encoded MSG-------"+session)
response.sendRedirect("https://pgi.billdesk.com/pgidsk/PGIMerchantPayment?msg="+strMsg);
%>
</script>
